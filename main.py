...
def ethstats(update, context):
    if(is_allowed):
        ...
        try:
            ...
            miner_id = ''
        if miner_id == '':
            context.bot.send_message(chat_id=update.message.chat_id, reply_to_message_id=update.message.message_id, text=f'Provide the miner ID to get the details from ethminer.org')
        else:    
